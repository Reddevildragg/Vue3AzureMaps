...
export class GeolocationControl implements atlas.Control {
  /****************************
   * Private Properties
   ***************************/
  ...
  private _gpsDotIcon =
    '<div class="atlas-map-gpsPulseIcon" style="background-color:{color}"></div>'
  ...
  public onAdd(map: atlas.Map, options?: atlas.ControlOptions): HTMLElement {
    ...
    this._resource = GeolocationControl._getTranslations(
  ...
  public onRemove(): void {
    ...
    if (typeof this._watchId !== 'undefined') {
      navigator.geolocation.clearWatch(this._watchId as number)
    }
  ...
  private _onGpsSuccess = (position: Position) => {
    ...
    if (this._isActive) {
      ...
      if (this._updateMapCamera) {
        ...
        const mapZoom = this._map.getCamera().zoom
  ...
  private static _getTranslations(lang?: string): GeolocationTranslation {
    ...
    switch (lang.toLowerCase()) {
      ...
      case 'af':
        return {
          enableTracking: 'begin dop',
          disableTracking: 'stop die dop',
          myLocation: 'my plek',
          title: 'ligginggewing beheer',
        }
      //Arabic
      case 'ar':
        return {
          enableTracking: 'بدء تتبع',
          disableTracking: 'تتبع توقف',
          myLocation: 'موقعي',
          title: 'السيطرة تحديد الموقع الجغرافي',
        }
      //Basque
      case 'eu':
        return {
          enableTracking: 'Hasi segimendua',
          disableTracking: 'Stop jarraipena',
          myLocation: 'Nire kokapena',
          title: 'Geokokapen kontrol',
        }
      //Bulgarian
      case 'bg':
        return {
          enableTracking: 'Започнете да проследявате',
          disableTracking: 'Спиране на проследяването',
          myLocation: 'Моето място',
          title: 'контрол за геолокация',
        }
      //Chinese
      case 'zh':
        return {
          enableTracking: '开始跟踪',
          disableTracking: '停止追踪',
          myLocation: '我的位置',
          title: '地理位置控制',
        }
      //Croatian
      case 'hr':
        return {
          enableTracking: 'Započnite praćenje',
          disableTracking: 'zaustavljanje praćenje',
          myLocation: 'Moja lokacija',
          title: 'kontrola Geolocation',
        }
      //Czech
      case 'cs':
        return {
          enableTracking: 'začít sledovat',
          disableTracking: 'Zastavit sledování',
          myLocation: 'Moje lokace',
          title: 'ovládání Geolocation',
        }
      //Danish
      case 'da':
        return {
          enableTracking: 'Start sporing',
          disableTracking: 'Stop sporing',
          myLocation: 'min placering',
          title: 'Geolocation kontrol',
        }
      //Dutch
      case 'nl':
        return {
          enableTracking: 'beginnen met het bijhouden',
          disableTracking: 'stop volgen',
          myLocation: 'Mijn locatie',
          title: 'Geolocation controle',
        }
      //Estonian
      case 'et':
        return {
          enableTracking: 'Alusta jälgimist',
          disableTracking: 'Stopp jälgimise',
          myLocation: 'Minu asukoht',
          title: 'Geolocation kontrolli',
        }
      //Finnish
      case 'fi':
        return {
          enableTracking: 'Aloita seuranta',
          disableTracking: 'Lopeta seuranta',
          myLocation: 'Minun sijaintini',
          title: 'Geolocation ohjaus',
        }
      //French
      case 'fr':
        return {
          enableTracking: 'Démarrer le suivi',
          disableTracking: "suivi d'arrêt",
          myLocation: 'Ma position',
          title: 'le contrôle de géolocalisation',
        }
      //Galician
      case 'gl':
        return {
          enableTracking: 'comezar a controlar',
          disableTracking: 'seguimento parada',
          myLocation: 'A miña localización',
          title: 'control de xeolocalización',
        }
      //German
      case 'de':
        return {
          enableTracking: 'starten Sie Tracking',
          disableTracking: 'Stop-Tracking',
          myLocation: 'Mein Standort',
          title: 'Geolokalisierung Steuer',
        }
      //Greek
      case 'el':
        return {
          enableTracking: 'Ξεκινήστε την παρακολούθηση',
          disableTracking: 'Διακοπή παρακολούθησης',
          myLocation: 'Η τοποθεσία μου',
          title: 'ελέγχου geolocation',
        }
      //Hindi
      case 'hi':
        return {
          enableTracking: 'ट्रैक करना शुरू',
          disableTracking: 'बंद करो ट्रैकिंग',
          myLocation: 'मेरा स्थान',
          title: 'जियोलोकेशन नियंत्रण',
        }
      //Hungarian
      case 'hu':
        return {
          enableTracking: 'követés indítása',
          disableTracking: 'követés leállítása',
          myLocation: 'Saját hely',
          title: 'Geolocation ellenőrzés',
        }
      //Indonesian
      case 'id':
        return {
          enableTracking: 'Mulai pelacakan',
          disableTracking: 'berhenti pelacakan',
          myLocation: 'Lokasi saya',
          title: 'kontrol geolocation',
        }
      //Italian
      case 'it':
        return {
          enableTracking: 'Inizia il monitoraggio',
          disableTracking: 'monitoraggio arresto',
          myLocation: 'La mia posizione',
          title: 'controllo geolocalizzazione',
        }
      //Japanese
      case 'ja':
        return {
          enableTracking: '追跡を開始',
          disableTracking: '追跡を停止',
          myLocation: '私の場所',
          title: 'ジオロケーション制御',
        }
      //Kazakh
      case 'kk':
        return {
          enableTracking: 'қадағалау бастау',
          disableTracking: 'қадағалау тоқтату',
          myLocation: 'Менің орналасуы',
          title: 'геоорын бақылау',
        }
      //Korean
      case 'ko':
        return {
          enableTracking: '추적 시작',
          disableTracking: '정지 추적',
          myLocation: '내 위치',
          title: '위치 정보 제어',
        }
      //Spanish
      case 'es':
        return {
          enableTracking: 'iniciar el seguimiento',
          disableTracking: 'Detener el seguimiento',
      ...
      case 'lv':
        return {
          enableTracking: 'Sākt izsekošana',
          disableTracking: 'Stop izsekošana',
          myLocation: 'Mana atrašanās vieta',
          title: 'Geolocation kontrole',
        }
      //Lithuanian
      case 'lt':
        return {
          enableTracking: 'pradėti stebėti',
          disableTracking: 'Sustabdyti sekimo',
          myLocation: 'Mano vieta',
          title: 'Geografinė padėtis kontrolė',
        }
      //Malay
      case 'ms':
        return {
          enableTracking: 'mula menjejaki',
          disableTracking: 'Stop pengesanan',
          myLocation: 'Lokasi saya',
          title: 'kawalan geolokasi',
        }
      //Norwegian
      case 'nb':
        return {
          enableTracking: 'begynne å spore',
          disableTracking: 'stopp sporing',
          myLocation: 'Min posisjon',
          title: 'geolocation kontroll',
        }
      //Polish
      case 'pl':
        return {
          enableTracking: 'rozpocząć śledzenie',
          disableTracking: 'Zatrzymaj śledzenie',
          myLocation: 'Moja lokacja',
          title: 'kontrola Geolokalizacja',
        }
      //Portuguese
      case 'pt':
        return {
          enableTracking: 'começar a controlar',
          disableTracking: 'rastreamento parada',
          myLocation: 'Minha localização',
          title: 'controle de geolocalização',
        }
      //Romanian
      case 'ro':
        return {
          enableTracking: 'Pornire urmărire',
          disableTracking: 'Oprire urmărire',
          myLocation: 'Locatia mea',
          title: 'controlul de geolocalizare',
        }
      //Russian
      case 'ru':
        return {
          enableTracking: 'Начать отслеживание',
          disableTracking: 'остановка отслеживания',
          myLocation: 'Мое местонахождение',
          title: 'контроль геолокации',
        }
      //Serbian
      case 'sr':
        return {
          enableTracking: 'Старт трацкинг',
          disableTracking: 'стоп праћење',
          myLocation: 'Моја локација',
          title: 'kontrola геолоцатион',
        }
      //Slovak
      case 'sk':
        return {
          enableTracking: 'začať sledovať',
          disableTracking: 'zastaviť sledovanie',
          myLocation: 'moja poloha',
          title: 'ovládanie Geolocation',
        }
      //Slovenian
      case 'sl':
        return {
          enableTracking: 'Začni sledenje',
          disableTracking: 'Stop za sledenje',
          myLocation: 'moja lokacija',
          title: 'nadzor Geolocation',
        }
      //Swedish
      case 'sv':
        return {
          enableTracking: 'börja spåra',
          disableTracking: 'Stoppa spårning',
      ...
      case 'th':
        return {
          enableTracking: 'เริ่มการติดตาม',
          disableTracking: 'ติดตามหยุด',
          myLocation: 'ตำแหน่งของฉัน',
          title: 'ควบคุม Geolocation',
        }
      //Turkish
      case 'tr':
        return {
          enableTracking: 'izlemeyi başlat',
          disableTracking: 'Dur izleme',
          myLocation: 'Benim konumum',
          title: 'Coğrafi Konum kontrolü',
        }
      //Ukrainian
      case 'uk':
        return {
          enableTracking: 'почати відстеження',
          disableTracking: 'зупинка відстеження',
          myLocation: 'моє місце розташування',
          title: 'контроль геолокації',
        }
      //Vietnamese
      case 'vi':
        return {
          enableTracking: 'Bắt đầu theo dõi',
          disableTracking: 'dừng theo dõi',
          myLocation: 'vị trí của tôi',
          title: 'kiểm soát định vị',
        }
      //English
      case 'en':
      default:
